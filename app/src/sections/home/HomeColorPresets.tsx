...
import { changeColorPreset } from '@src/store/settings/settings.slice';
import { ColorPreset } from '@src/store/settings/types';
...
export default function HomeColorPresets() {
  ...
  const themeColorPresets = useAppSelector(
    state => state.settings.themeColorPresets,
  );
  const currentThemeColorPreset = useAppSelector(
    ...
    <RootStyle>
      ...
      >
        ...
        <RadioGroup
          ...
          onChange={e =>
            dispatch(
              changeColorPreset(e.target.value as unknown as ColorPreset),
            )
          }
          sx={{ my: 5 }}
        >
